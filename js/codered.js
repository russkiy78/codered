...
Codered.prototype.createKey = function () {
    ...
    up:   do {
        ...
        for (var i = 0; i < this.circles; i++) {
            ...
            this.privatekey.q[i] = this.nextPrime(Math.max.apply(null, this.openkey) * this.qfrommax, Math.floor(Math.random() * this.maxq));
    ...
    this.evensorted = this.evensorted.sort(function (a, b) {
        return a - b;
    });
...
Codered.prototype.decode = function (data) {
    ...
    /**/
    if (this.minimize) {
        for (var i = 0; i < this.decodeplus; i++) {
